...
class UPBConnect:
    def __init__(self, root):
        ...
        # --- NUEVAS VARIABLES DE PERSISTENCIA DE SESIÓN LOCAL ---
        self.sesion_activa = False
        self.nombre_usuario = ""
        self.correo_usuario = ""

        # Almacen marcador and linea de trayecto con animaciones
    ...
    def mostrar_pantalla_bienvenida(self):
        ...
        lbl_titulo = tk.Label(frame_contenido, text="UPBConnect", font=("Segoe UI", 28, "bold"), fg=color_upbc_morado, bg="#ffffff")
        lbl_titulo.pack(pady=(0, 2))

        #condicion por si ya se registro
        if self.sesion_activa:
            lbl_subtitulo = tk.Label(frame_contenido, text="¡Bienvenido de vuelta!", font=("Segoe UI", 13, "bold"), fg="#333333", bg="#ffffff")
            lbl_subtitulo.pack(pady=(15, 5))

            lbl_usuario_info = tk.Label(frame_contenido, text=self.nombre_usuario, font=("Segoe UI", 12), fg=color_upbc_morado, bg="#ffffff")
            lbl_usuario_info.pack(pady=(0, 2))
            
            lbl_correo_info = tk.Label(frame_contenido, text=self.correo_usuario, font=("Segoe UI", 9, "italic"), fg="#777777", bg="#ffffff")
            lbl_correo_info.pack(pady=(0, 25))

            btn_reingresar = tk.Button(
                frame_contenido, text="Entrar al Mapa", font=("Segoe UI", 12, "bold"), 
                bg=color_upbc_morado, fg="white", activebackground="#4A148C", activeforeground="white",
                bd=0, cursor="hand2", command=self.mostrar_pantalla_mapa
            )
            btn_reingresar.pack(fill="x", ipady=6, pady=5)

            btn_cambiar_user = tk.Button(
                frame_contenido, text="Cambiar de Usuario", font=("Segoe UI", 10, "bold"), 
                bg="#eeeeee", fg="#555555", activebackground="#cccccc", activeforeground="#333333",
                bd=1, relief="solid", cursor="hand2", command=self.cerrar_sesion_local
            )
            btn_cambiar_user.pack(fill="x", ipady=4, pady=5)

        #condicion si es la primera vez
        else:
            ...
            #caja correo electronico
            lbl_correo = tk.Label(frame_contenido, text="Correo electrónico:", font=("Segoe UI", 10, "bold"), fg="#444444", bg="#ffffff")
            lbl_correo.pack(anchor="w")
            self.txt_correo = tk.Entry(frame_contenido, font=("Segoe UI", 11), bd=1, relief="solid")
            self.txt_correo.pack(fill="x", pady=(2, 20), ipady=4)

            # Botón de ingreso
            btn_ingresar = tk.Button(
                frame_contenido, text="Registrar/Ingresar", font=("Segoe UI", 12, "bold"), 
                bg=color_upbc_morado, fg="white", activebackground="#4A148C", activeforeground="white",
    ...
    def registrar_alumno_sql(self):
        """Valida las reglas de sintaxis del formulario e inserta la fila en SQL Server."""
        nombre = self.txt_nombre.get().strip()
        correo = self.txt_correo.get().strip()

        #validacion de datos
        if not nombre or not correo:
            messagebox.showwarning("Campos Incompletos", "Por favor, escribe tu nombre y correo electrónico antes de ingresar.")
            return
        ...
        if not re.match(patron_nombre, nombre):
            messagebox.showerror(
                "Nombre Inválido", 
                "El apartado de nombre completo no permite el ingreso de números ni símbolos especiales."
            )
            return
        ...
        if not re.match(patron_correo, correo):
            messagebox.showerror(
                "Correo Inválido", 
                "La sintaxis del correo electrónico es incorrecta.\n\nAsegúrate de que incluya el símbolo '@', un dominio válido y no contenga espacios en blanco."
            )
            return
        ...
        if conexion:
            try:
                ...
                cursor.execute(
                    "INSERT INTO Alumnos_Registro (correo_electronico, nombre_alumno) VALUES (?, ?)",
                    (correo, nombre)
                )
                conexion.commit()
                cursor.close()
    ...
    def cerrar_sesion_local(self):
        """Limpia los almacenes de sesión para permitir el registro de una cuenta nueva."""
        self.sesion_activa = False
        self.nombre_usuario = ""
        self.correo_usuario = ""
        self.mostrar_pantalla_bienvenida()
    ...
    def animar_movimiento_autobus(self):
        ...
        # (Este es el tramo final del método animar_movimiento_autobus)
